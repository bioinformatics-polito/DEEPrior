...
def evaluate_fusions(list_fusions, trained_model_path, output_file_path):
    ...
    toskip = [f for f, fusion in enumerate(list_fusions) if (fusion == 'Missing information, fusion skipped' or (
        fusion.chr5p == 'Invalid value' or fusion.chr3p == 'Invalid value'))]
    for fusion in list_fusions:
        i = i+1
        print("Deep learning processing: %0.2f%% completed" %
              (i/len(list_fusions)*100))
        if (i-1) in toskip:
            ...
            x_test, sequences, label = retrieve_test_sequences(
                fusion, tokenizer)
            if label == -1:
            ...
            if len(x_test) > 0:
                ...
                predictions = [pred[0].astype(np.float)
                ...
                try:
                    ...
                    df_res = df_res.append({'FusionPair': '',
                                            'OncogenicProbability': '',
                                            'Version': '',
                                            'Chr5p': '',
                                            'Coord5p': '',
                                            '5pStrand': '',
